...
class A:
	...
	def login_click(self):
		w,utype=Authentication.loginCheck(self.v1.get(),self.v2.get())
		
		if (w!=-1):
			if (self.v2.get()=='anamika'):
				self.root.destroy()
				main_frame.Change_password(w)
			elif (utype=='Admin'):
				self.root.destroy()
				main_frame.B(w,utype)
			else:
				messagebox.showerror('ACCESS PERMISSION','Permission to Admins only')
